  ...
  return <div className="shopping-module">
    ...
    <section className="shopping-board">
      {SECTIONS.map((section) => {
        ...
        return <article className={`shopping-column ${section.status}`} key={section.status}><header><div><p className="section-kicker">{section.eyebrow}</p><h3>{section.title}</h3><span>{section.description}</span></div><strong>{sectionItems.length}</strong></header><div className="shopping-list">{sectionItems.map((item) => <div className="shopping-row" key={item.id}><button className="shopping-check" onClick={() => toggleBought(item)} aria-label={`标记已买：${item.title}`}>✓</button><button className="shopping-row-main" onClick={() => setEditor(item)}><strong>{item.title}</strong>{item.quantity && <span>{item.quantity}</span>}<small>{item.category}{item.details ? ` · ${item.details}` : ""}</small></button><button className="shopping-row-delete" onClick={() => onChange(items.filter((current) => current.id !== item.id))} aria-label={`删除：${item.title}`}>×</button></div>)}{sectionItems.length === 0 && <button className="shopping-empty-add" onClick={() => { setQuickStatus(section.status); setEditor("new"); }}>＋ 添加到这里</button>}</div></article>;
      })}
    </section>
