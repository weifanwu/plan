...
export default function Home() {
  ...
  function saveQuickTask(title: string, time: string, allDay: boolean) {
    if (!quickTaskDate || !title.trim()) return;
    const task: Task = { id: uid(), title: title.trim(), details: null, category: "生活", date: quickTaskDate, time: allDay ? null : time || null, endDate: null, carriedFrom: null, completedAt: null, goalId: null, priority: "normal", status: "todo" };
            ...
            <section className="panel routine-manager">
              ...
              <div className="routine-list">
                {data.routines.map((routine) => <article className={`routine-row ${routine.active ? "" : "paused"}`} key={routine.id}>
                  ...
                  <div className="routine-row-actions"><button onClick={() => toggleRoutineActive(routine.id)}>{routine.active ? "暂停" : "恢复"}</button><button onClick={() => setRoutineEditor(routine)}>编辑</button><SafeDeleteButton onConfirm={() => deleteRoutine(routine.id)}>删除</SafeDeleteButton></div>
                </article>)}
            ...
            {notesMode === "backlog" ? <section className="draft-workbench">
              ...
              <section className="draft-inbox">
                ...
                {visibleNotes.length > 0 ? <div className="draft-list">{visibleNotes.map((note) => <article className={`draft-row note-${note.category} ${note.pinned ? "pinned" : ""}`} key={note.id}>
                  ...
                  <div className="draft-row-actions"><button className="draft-schedule" onClick={() => promoteNoteToTask(note)}>安排成任务</button><SafeDeleteButton className="draft-delete" confirmLabel="确认" onConfirm={() => removeRecord("notes", note.id, `已删除草稿「${noteTitle(note)}」`)} aria-label={`删除草稿：${noteTitle(note)}`} title="删除草稿">×</SafeDeleteButton></div>
...
function TaskRow({ task, goal, onToggle, onTogglePriority, onEdit, onDelete, compact = false, selectionMode = false, selected = false, onSelect }: { task: Task; goal?: Goal; onToggle: () => void; onTogglePriority?: () => void; onEdit: () => void; onDelete: () => void; compact?: boolean; selectionMode?: boolean; selected?: boolean; onSelect?: () => void }) {
  ...
  return <div className={`task-row ${task.status === "done" ? "done" : ""} ${carried ? "carried" : ""} ${compact ? "compact" : ""} ${selectionMode ? "selecting" : ""} ${selected ? "selected" : ""}`}>
    ...
    <div className="task-actions">{onTogglePriority && <button className={task.priority === "high" ? "priority-active" : ""} onClick={onTogglePriority}>{task.priority === "high" ? "取消重点" : "设为重点"}</button>}<button onClick={onEdit}>编辑</button><SafeDeleteButton onConfirm={onDelete}>删除</SafeDeleteButton></div>
  </div>;
}
...
function QuickTaskModal({ date, onClose, onSave }: { date: string; onClose: () => void; onSave: (title: string, time: string, allDay: boolean) => void }) {
  const [title, setTitle] = useState("");
  const [time, setTime] = useState("09:00");
  const [allDay, setAllDay] = useState(false);
  const inputRef = useRef<HTMLInputElement>(null);
  useEffect(() => { inputRef.current?.focus(); }, []);
  return <div className="modal-backdrop quick-task-backdrop"><section className="quick-task-modal" aria-label={`快速添加 ${formatDate(date)} 的任务`}><header><div><p className="section-kicker">QUICK TASK</p><h2>{formatDate(date)}</h2></div><button onClick={onClose} aria-label="关闭">×</button></header><form onSubmit={(event) => { event.preventDefault(); onSave(title, time, allDay); }}><input ref={inputRef} value={title} onChange={(event) => setTitle(event.target.value)} placeholder="任务名称" aria-label="任务名称" /><div className="quick-task-timing"><div className="quick-time-mode" role="group" aria-label="任务时间类型"><button type="button" className={allDay ? "active" : ""} aria-pressed={allDay} onClick={() => setAllDay(true)}>全天</button><button type="button" className={!allDay ? "active" : ""} aria-pressed={!allDay} onClick={() => setAllDay(false)}>指定时间</button></div>{!allDay && <label><span>具体时间</span><input type="time" value={time} onChange={(event) => setTime(event.target.value)} /></label>}<button className="primary-button" type="submit" disabled={!title.trim()}>创建</button></div></form><p>全天任务不会绑定具体时刻；需要补充细节、类别、跨度或目标时，再点击任务打开完整编辑。</p></section></div>;
}

function TaskTimeField({ allDay, time, onAllDayChange, onTimeChange }: { allDay: boolean; time: string; onAllDayChange: (value: boolean) => void; onTimeChange: (value: string) => void }) {
  return <fieldset className="task-time-field"><legend>时间</legend><div className="task-time-mode" role="group" aria-label="任务时间类型"><button type="button" className={allDay ? "active" : ""} aria-pressed={allDay} onClick={() => onAllDayChange(true)}>全天</button><button type="button" className={!allDay ? "active" : ""} aria-pressed={!allDay} onClick={() => onAllDayChange(false)}>指定时间</button></div>{!allDay && <input type="time" value={time} onChange={(event) => onTimeChange(event.target.value)} aria-label="具体时间" />}</fieldset>;
}
