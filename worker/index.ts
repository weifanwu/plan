...
4. 求职记录: a Kanban pipeline with exactly four stages: 已投, 面试, Offer, 拒绝. Each application stores company, role, job link, location, contact, application date, the original job description, and the user's notes/next step. LinkedIn imports may also carry source=linkedin, externalJobId, capturedAt, and stageHistory. The date powers daily and weekly summaries, so preserve the actual application date.
...
- Extract company, role, the most relevant job link, location, useful context, requirements, and a concrete next step. Preserve the original job description in application.description; put only the user's evaluation, follow-up, and concise supporting context in application.notes. Preserve supplied URLs exactly.
- Application shape: {id,company,role,stage,link,location,contact,date,description,notes,source,externalJobId,capturedAt,stageHistory}. For ordinary AI-created records use source=ai, blank externalJobId/capturedAt when unknown, and a stageHistory array containing the initial stage and a valid ISO timestamp. Preserve existing source and capture metadata on updates unless explicitly asked to replace them.
